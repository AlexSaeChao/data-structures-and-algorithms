...
const longestString = (arr) => {
  ...
  let longestIndex = -1;
  arr.every((str, index) => {
    if (longestIndex === -1 || str.length > arr[longestIndex].length) {
      longestIndex = index;
    }
    return true;
  });
...
const firstLetters = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    const firstLetter = arr[i][0]; // Get the first character of the string

    result.push(firstLetter); // Add the first letter to the result array
  }
